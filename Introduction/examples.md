...
这些例子同时说明了一些十分基础容易被人忽略的特性。都包括一个活跃的进行决策的代理与其周围环境的*交互*，同时代理也在这样*不确定*的环境中寻找实现自己*目标*的方法。代理的行为会对未来的环境状态产生影响（比如下一步棋、精炼厂的储量、机器人的下一个位置与未来的电量），这些环境变化又会进一步的影响未来代理的选择和机会。正确的选择需要考虑间接的、延迟的一系列行动，还可能会需要预见和规划的能力。
